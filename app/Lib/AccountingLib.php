...
/*
        $revenueOrders = Order::with(['orderTickets' => function($orderTicket){
                            ...
                            ->select(function ($query) use ($dateFormat) {
                                $query->selectRaw('SUM(orders.finalPrice) as revenue');
                                $query->selectRaw('SUM(orders.adultQuantity) as pax');
                                $query->selectRaw("SUM(CASE WHEN op.paymentStatus = 8 THEN orders.finalPrice ELSE 0 END) as paid");
                                $query->selectRaw("SUM(CASE WHEN op.paymentStatus = 0 THEN orders.finalPrice ELSE 0 END) as notpaid");
                                $query->selectRaw("DATE_FORMAT(orders.bookingDate, '{$dateFormat}') as data");

                                $pms = PaymentMethod::get();
                                foreach($pms as $pm){
                                    $query->selectRaw("SUM(CASE WHEN pm.name = '{$pm->name}' THEN orders.finalPrice ELSE 0 END) as '{$pm->name}'");
                                }
                            })
